...
export default function AppLayout({ children }) {
  ...
  return (
    <div className="flex h-screen bg-[#070b14] text-slate-100 overflow-hidden font-sans">
      ...
      <aside className="hidden md:flex flex-col w-64 border-r border-slate-800/80 bg-[#0b101d]/95 backdrop-blur-xl z-20 shrink-0 h-full">
        {/* Brand Header */}
        ...
        <div className="p-3 border-t border-slate-800/60 bg-slate-900/60 shrink-0 sticky bottom-0">
          <div className="flex items-center justify-between p-2 rounded-xl bg-slate-800/40 border border-slate-700/40">
            ...
            {!isClerkConfigured && (
              <button
                ...
                title="Sign Out"
                className="p-2 text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-lg transition-colors cursor-pointer shrink-0 ml-1"
              >
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex flex-col min-w-0 h-full overflow-hidden bg-gradient-to-b from-[#070b14] via-[#090e1a] to-[#060a12]">
        {/* Top Navbar */}
        <header className="h-14 border-b border-slate-800/60 bg-[#070b14]/90 backdrop-blur-md flex items-center justify-between px-4 sm:px-6 z-10 shrink-0">
          <div className="flex items-center gap-3 min-w-0">
            <button
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              className="md:hidden p-2 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800/60 cursor-pointer"
            >
          ...
          <div className="flex items-center gap-2.5 sm:gap-3">
            <div className="hidden xl:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-medium shrink-0">
            ...
            {/* Top Navbar User & Logout (Always Accessible across all resolutions) */}
            <div className="flex items-center gap-2 pl-2 border-l border-slate-800/80">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-medium text-slate-300 hidden lg:inline truncate max-w-[120px]">
                  {userName}
                </span>
              </div>
              <button
                onClick={handleLogout}
                title="Sign Out"
                className="px-2.5 py-1.5 rounded-lg text-xs font-semibold text-rose-300 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/20 hover:border-rose-500/40 flex items-center gap-1.5 transition-all cursor-pointer shrink-0"
              >
        ...
        {/* Mobile Dropdown Nav */}
        {mobileMenuOpen && (
          <div className="md:hidden border-b border-slate-800 bg-[#0b101d] px-4 py-3 space-y-1 z-30 shrink-0 shadow-2xl">
            <a
              href="/downloads/lifeops-ai-chrome-extension.zip"
              download="lifeops-ai-chrome-extension.zip"
            ...
            <button
              onClick={handleLogout}
              className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-rose-400 hover:bg-rose-500/10 cursor-pointer"
            >
              <LogOut className="w-4 h-4" />
              <span>Sign Out</span>
            </button>
