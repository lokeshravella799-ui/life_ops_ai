...
export default function AdaptiveReplanModal({ isOpen, onClose, workflowId, onReplanSuccess }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/75 backdrop-blur-md overflow-y-auto">
      <div className="relative w-full max-w-3xl bg-[#0d1322] border border-indigo-500/30 rounded-2xl shadow-2xl p-6 overflow-hidden">
        {/* Glow Header */}
        ...
        {!replanResult ? (
          <form onSubmit={handleReplanSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-4 py-3 rounded-xl bg-slate-900/90 border border-slate-700/80 text-sm text-white placeholder-slate-500 focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500"
                required
              />
            </div>
            ...
            <div className="flex flex-wrap gap-2 pt-1">
              ...
              {[
                ...
                <button
                  ...
                  className="px-2.5 py-1 rounded-lg bg-slate-800/60 hover:bg-slate-800 text-[11px] text-slate-300 border border-slate-700/60 transition-colors"
                >
                  {preset}
                </button>
              ))}
            ...
            <div className="flex justify-end gap-3 pt-4 border-t border-slate-800">
              ...
              <button
                type="submit"
                disabled={loading}
                className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-semibold flex items-center gap-2 shadow-lg shadow-purple-600/30 disabled:opacity-50 transition-all"
              >
                {loading ? (
                  <>
          ...
          <div className="space-y-4">
            <div className="p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-xs text-emerald-300 flex items-center gap-2.5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800/80">
                <h4 className="text-xs font-bold text-slate-400 uppercase mb-2">Original Plan Overview</h4>
              ...
              <div className="p-4 rounded-xl bg-indigo-950/40 border border-indigo-500/30">
                <h4 className="text-xs font-bold text-indigo-300 uppercase mb-2">New Adapted Plan Overview</h4>
                <p className="text-xs text-slate-200 leading-relaxed font-medium">
            ...
            {replanResult.replanData?.changesMade?.length > 0 && (
              <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800">
                <h4 className="text-xs font-bold text-slate-300 mb-2">Specific Adjustments Applied:</h4>
            ...
            <div className="flex justify-end gap-3 pt-3 border-t border-slate-800">
              <button
                onClick={onClose}
                className="px-5 py-2.5 rounded-xl bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-semibold transition-all shadow-lg shadow-indigo-600/30"
              >
