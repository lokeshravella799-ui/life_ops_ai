  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div>
        ...
        <h1 className="text-2xl sm:text-3xl font-extrabold text-white">Customer Support & Refund Triage</h1>
      ...
      {/* Input Studio Card */}
      <div className="p-6 sm:p-8 rounded-3xl bg-[#0d1424]/90 border border-purple-500/20 shadow-xl space-y-5">
        <form onSubmit={handleTriage} className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-700 text-sm text-white focus:outline-none focus:border-purple-500 font-mono"
                required
              />
            </div>
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full px-4 py-3 rounded-2xl bg-slate-900 border border-slate-700 text-sm text-white focus:outline-none focus:border-purple-500 resize-none font-normal leading-relaxed"
              required
            />
          </div>
          ...
          <div className="flex justify-end pt-2">
            <button
              ...
              className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-500 hover:to-indigo-500 text-white text-xs font-semibold flex items-center gap-2 shadow-lg shadow-purple-600/30 transition-all disabled:opacity-50"
            >
              {loading ? (
                <>
      ...
      {triageResult && (
        <div className="p-6 sm:p-8 rounded-3xl bg-[#0d1424] border border-purple-500/30 shadow-2xl space-y-6 animate-in fade-in zoom-in-95 duration-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl bg-slate-900/80 border border-slate-800 space-y-1">
              <h4 className="text-xs font-bold text-slate-400 uppercase">Policy Rationale</h4>
              <p className="text-xs text-slate-200 leading-relaxed">{triageResult.policyRationale}</p>
            </div>
            <div className="p-4 rounded-2xl bg-indigo-950/40 border border-indigo-500/30 space-y-1">
              <h4 className="text-xs font-bold text-indigo-300 uppercase">Recommended Resolution</h4>
              <p className="text-xs text-slate-200 leading-relaxed font-semibold">{triageResult.recommendedResolution}</p>
            </div>
          </div>
          ...
          <div className="p-5 rounded-2xl bg-slate-900/90 border border-slate-800 space-y-3">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="px-3 py-1 rounded-lg bg-slate-800 hover:bg-slate-700 text-[11px] font-semibold text-slate-200 flex items-center gap-1.5 transition-colors"
              >
                {copied ? <Check className="w-3.5 h-3.5 text-emerald-400" /> : <Copy className="w-3.5 h-3.5" />}
                <span>{copied ? 'Copied to Clipboard' : 'Copy Email'}</span>
              </button>
            </div>
            <div className="p-4 rounded-xl bg-slate-950/90 border border-slate-800/80 text-xs text-slate-300 font-mono whitespace-pre-wrap leading-relaxed">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {triageResult.internalActionTasks?.map((task, idx) => (
                ...
                >
                  ...
                  <span className="text-[10px] text-slate-400">Est. duration: {task.estimatedHours}h</span>
                </div>
              ))}
            </div>
          </div>
