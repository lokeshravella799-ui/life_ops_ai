...
export default function TasksPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="p-5 sm:p-6 rounded-3xl bg-gradient-to-br from-[#111728] via-[#0E121E] to-[#0A0D14] border border-indigo-500/20 shadow-xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3 text-xs">
          <div className="flex flex-wrap items-center gap-4 sm:gap-6">
            <div className="flex items-center gap-1.5">
              <Target className="w-4 h-4 text-indigo-400" />
              <span className="text-slate-200 font-semibold">{goals.length} Goals</span>
            </div>
            <div className="flex items-center gap-1.5">
          ...
          <span className="font-mono font-bold text-emerald-400 text-sm">{globalProgress}% Completed</span>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 p-3.5 rounded-2xl bg-[#0d1424]/80 border border-slate-800">
        {/* Search */}
        <div className="flex items-center gap-2 flex-1 min-w-[220px] bg-slate-900/90 rounded-xl px-3 py-2 border border-slate-800">
      ...
      {loading && (
        <div className="flex flex-col items-center justify-center py-16 space-y-3">
          ...
          <p className="text-xs text-slate-400">Loading Central Execution Roadmap...</p>
        </div>
      )}
      ...
      {!loading && goals.length === 0 && (
        <div className="p-12 text-center rounded-3xl bg-slate-900/40 border border-slate-800/80 space-y-5">
          ...
          <div>
            <h3 className="text-base font-bold text-white">Your Roadmap is Empty</h3>
        ...
        <div className="space-y-6">
          {goals.map(goal => {
            ...
            return (
              <div
                key={goal.id}
                className="rounded-3xl bg-[#0d1424]/90 border border-slate-800/80 overflow-hidden shadow-xl"
              >
                {/* Goal Header Bar */}
                <div className="p-5 sm:p-6 bg-slate-900/70 border-b border-slate-800/60 space-y-4">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                    <div className="flex items-start sm:items-center gap-3">
                      <button
                        onClick={() => toggleGoalCollapse(goal.id)}
                        className="p-1 rounded-lg text-slate-400 hover:text-white hover:bg-slate-800 transition-colors cursor-pointer mt-0.5 sm:mt-0"
                    ...
                    <div className="flex items-center gap-2 self-end sm:self-auto">
                      ...
                      <button
                        ...
                        className="px-3.5 py-1.5 rounded-xl bg-indigo-600/20 hover:bg-indigo-600 text-indigo-300 hover:text-white text-xs font-semibold flex items-center gap-1 border border-indigo-500/30 transition-all cursor-pointer"
                      >
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <span className="font-mono font-bold text-indigo-300">{goalProgress}%</span>
                    </div>
                ...
                {addingTaskGoalId === goal.id && (
                  <div className="p-4 bg-slate-900/90 border-b border-indigo-500/30 space-y-3 animate-in fade-in zoom-in-95 duration-200">
                    <h4 className="text-xs font-bold text-white">Add Task to {goal.title}</h4>
                    ...
                    <div className="flex items-center justify-between gap-2 pt-1">
                      <select
                        ...
                        className="px-3 py-1.5 rounded-xl bg-slate-950 border border-slate-700 text-xs text-white focus:outline-none cursor-pointer"
                      >
                        <option value="HIGH">🔴 High Priority</option>
                        <option value="MEDIUM">🟡 Medium Priority</option>
                        <option value="LOW">🟢 Low Priority</option>
                    ...
                    {goalTasks.length === 0 ? (
                      <div className="py-6 text-center space-y-3">
                        ...
                        {allGoalTasks.length === 0 && (
                          <button
                            onClick={() => navigate(`/goals/${goal.id}`)}
                            className="px-4 py-2 rounded-xl bg-indigo-600/20 hover:bg-indigo-600 text-indigo-300 hover:text-white text-xs font-semibold inline-flex items-center gap-1.5 border border-indigo-500/30 transition-all cursor-pointer"
                          >
                        ...
                        {goalTasks.map(task => {
                          ...
                          return (
                            <div
                              ...
                              className={`rounded-2xl border transition-all ${
                                isDone
                                  ...
                                  : isInProgress
                                  ? 'bg-[#0f172a]/90 border-amber-500/40 shadow-lg shadow-amber-950/20'
                            ...
                            >
                              ...
                              >
                                <div className="flex items-start gap-3 min-w-0">
                                  ...
                                  <div className="space-y-1 min-w-0">
                                    ...
                                    <p className={`text-xs font-bold leading-relaxed ${isDone ? 'line-through text-slate-500' : 'text-slate-200'}`}>
                                      {task.title}
                                    </p>
                                  </div>
                                </div>
                              ...
                              {isExpanded && (
                                <div className="p-4 sm:p-5 border-t border-slate-800/80 bg-slate-950/70 space-y-4 animate-in fade-in zoom-in-95 duration-150">
                                  {/* What to do */}
                                  <div>
                                    <h5 className="text-[11px] font-bold text-indigo-400 uppercase tracking-wider flex items-center gap-1.5 mb-1">
                                  ...
                                  {/* Why this matters */}
                                  <div>
                                    <h5 className="text-[11px] font-bold text-purple-400 uppercase tracking-wider flex items-center gap-1.5 mb-1">
